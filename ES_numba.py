...
@jit(nopython=True)
def eventsync(es1, es2,  taumax, ts1=None, ts2=None):  #taumax in Zeitschritten, im fall von täglichen daten ist ein Zeitschritt ein Tag / np.inf
    # Get time indices
    if ts1 is None:
        e1 = np.where(es1)[0]                         
    else:                                             
    ...
    count12 = np.sum((dst12 > 0) * (dst12 <= efftau)) + eqtime   #extreme-events happen on node 2 before happening on node 1
    count21 = np.sum((dst12 < 0) * (-dst12 <= efftau)) + eqtime  #extreme-events happen on node 1 before happening on node 2
...
#cleaning the event time series of consecutive events/ events on consecutive timesteps (6h) are considered as single events and are placed on the first timestep
@jit(nopython=True)
    ...
    for i in range(unique_num_of_events.shape[0]):
        for j in range(unique_num_of_events.shape[0]):
            for n in range(len(ES_surrogate_i_j)):
                sur1 = (np.random.random(size=6130)<(unique_num_of_events[i]/6130))   #6130 - temporal size of data and 308/6130 - prbability of an event
            ...
            thresh[i,j] = np.percentile(ES_surrogate_i_j, sign)  #diagonal elements are calculated twice... could be optimized with an if i == j condition
            thresh[j,i] = np.percentile(ES_surrogate_j_i, sign)
    return thresh

@jit(nopython=True)
def es_loop(clean_data, es, sign_es, size, rank, ts1, ts2, d, taumax): #clean_data should be the cleaned version of data
    ...
    for i in range(prange*rank, prange*(rank+1)):
        for j in range(i+1,clean_data.shape[0]):
            ...
            es[i,j] = es_val1
            es[j,i] = es_val2
            if es_val1 >= d[cantor_pairing(clean_data[i].sum(), clean_data[j].sum())]:    #str function causes errors with numba
                sign_es[i,j] = 1
            else:
                sign_es[i,j] = 0
...
def main():
    ...
    thresh_val = treshhold_params(thresh, ES_surrogate_i_j, ES_surrogate_j_i, num_of_events, time_steps_arr, time_steps_arr, taumax, sign)
    thresh_df = pd.DataFrame(data=thresh_val, index=num_of_events,  columns=num_of_events)    # values, 1st column as index, 1st row as the column names 
    ...
    if rank == 0:
        np.savetxt(file_path+goalDir+str(rank)+'_'+str(sign)+'_'+file_num_of_events, num_of_events)
        np.savetxt(file_path+goalDir+str(rank)+'_'+str(sign)+'_'+file_thresh, thresh_val)

    sparse.save_npz(file_path+goalDir+str(rank)+'_'+str(sign)+'_'+file_es, ev_sync_csr)
    sparse.save_npz(file_path+goalDir+str(rank)+'_'+str(sign)+'_'+file_sign_es, sign_ev_sync_csr)
